...
function BackDrops:_create_opts()
   -- Check if images array is empty or index is invalid
   if not self.images or #self.images == 0 or not self.images[self.current_idx] then
      -- Return only the color background without an image
      return {
         {
